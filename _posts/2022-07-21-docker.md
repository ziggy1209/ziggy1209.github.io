...
```
and then name it with 'docker tag <IMAGE ID> <IMAGE NAME>' for the ease of use.
...
To start a container using an existing image and mounting it to a folder in your local host, you can type
```bash
docker run -it --name <CONTAINER NAME TO BE GIVEN> --gpus all -v <LOCAL DIRECTORY>:/<MOUNTING PATH> <IMAGE ID>
```
Then the content of the container is mounted in your desired path - all the changes made in your local host will be synced in the container real-time.
