...
If you have any running/stopped container based on the image to be deleted, you will need to delete the containers by `docker rm <container id>` first.
...
Having made some changes to the current image environment in a running container, you can save the docker container state into a new image by `docker commit <CONTAINER ID> <NEW IMAGE NAME>:<TAG>`.
